...
**Type your name into the box and a flower grows from it.**

Every plant here is one number. The same number always grows the same plant, on any
computer, forever. There are no flower pictures anywhere in this repository. Nothing is
stored and nothing is downloaded. The number itself is turned into the plant, petal by
petal, every time you load the page.

There are about 4.3 billion possible plants across four species, from roughly 1,400 lines
of JavaScript with no libraries and no build step.

**Live:** https://darshikaroyal1307.github.io/
